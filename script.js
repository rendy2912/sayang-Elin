...
const sceneMultiWrap = document.getElementById('sceneMultiWrap');
...
yesBtn.addEventListener('click', () => {
  ...
  // "unlock" bgMusic2 di dalam gesture user yang sama, supaya nanti
  // bisa di-play() lewat timer tanpa diblokir browser (khususnya iOS Safari)
  bgMusic2.volume = 0;
  bgMusic2.play().then(() => {
    bgMusic2.pause();
...
/* --- slot posisi: cluster di sekitar tengah layar, cukup renggang
       biar teks yang muncul bareng gak saling tabrakan --- */
const SLOT_POOL = [
  { x: 0,   y: 0 },
  { x: -20, y: -13 },
  { x: 20,  y: -13 },
...
function pickSlots(count) {
  const shuffled = [...SLOT_POOL].sort(() => Math.random() - 0.5);
...
// kelompokkan 100 scene jadi batch berisi 1-3 teks yang tampil bersamaan
function buildBatches(list) {
  const batches = [];
  let i = 0;
  while (i < list.length) {
    let size = Math.floor(rand(1, 4)); // 1, 2, atau 3
    size = Math.min(size, list.length - i);
...
const sceneBatches = buildBatches(scenes);

const SCENE_DURATION = 300; // ms per batch
const MUSIC_FADE_OUT_MS = 1600; // music.mp3 melembut menjelang batch terakhir
...
function renderScene(batchIndex) {
  sceneMultiWrap.innerHTML = '';
  const batch = sceneBatches[batchIndex];
  const slots = pickSlots(batch.length);

  batch.forEach((text, i) => {
    const fx = pickFx(batchIndex + i);
    const slot = slots[i];

    const item = document.createElement('div');
    item.className = 'scene-item';
    item.style.left = `calc(50% + ${slot.x}vw)`;
    item.style.top = `calc(50% + ${slot.y}vh)`;

    const inner = document.createElement('span');
    inner.className = `scene-item-inner ${fx}`;
    inner.textContent = text;

    item.appendChild(inner);
    sceneMultiWrap.appendChild(item);
  });
}
...
function startSceneSequence() {
  ...
  const totalDuration = sceneBatches.length * SCENE_DURATION;

  function tick(now) {
    const elapsed = now - startTime;
    const targetIndex = Math.floor(elapsed / SCENE_DURATION);

    if (targetIndex !== sceneIndex && targetIndex < sceneBatches.length) {
    ...
    // mulai melembutkan music.mp3 menjelang batch terakhir
    ...
    if (sceneIndex >= sceneBatches.length - 1 && elapsed >= totalDuration) {
      finishSceneSequence();
      return;
    }
