...
const SCENE_DURATION = 500; // ms per scene
let sceneIndex = 0;
let sceneTimer = null;
...
function startSceneSequence() {
  ...
  function tick(now) {
    ...
    if (sceneIndex >= scenes.length - 1 && elapsed >= scenes.length * SCENE_DURATION) {
      finishSceneSequence();
      return;
    }
...
heartBtn.addEventListener('click', () => {
  ...
  // background fades to black, music fades out, everything disappears
  setTimeout(() => {
...
function fadeOutMusic() {
  ...
  const interval = setInterval(() => {
    step++;
    const newVol = Math.max(0, startVolume * (1 - step / steps));
